...
static uint8_t* extract_bytes(const vmcu_report_t *report, int32_t *size);
static int32_t count_bytes(const vmcu_report_t *report);

static void mkstr(vmcu_string_t *str, uint8_t *bytes, int32_t s, int32_t e);

/* --- Extern --- */

int vmcu_analyze_strings(vmcu_report_t *report) {

    int32_t size; int32_t s = -1, slen = 0;
