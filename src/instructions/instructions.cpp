...
void ld_yi(Sys *sys, int opcode) {
    ...
    int8_t data = sys->read_data((yh << 8) + yl);
    int16_t post_y = ((yh << 8) + yl) + 0x01;

    sys->write_gpr(dest, data);
    sys->write_gpr(YL, (0x00ff & post_y));
    sys->write_gpr(YH, (0xff00 & post_y) >> 8);
}

void ld_dy(Sys *sys, int opcode) {
    ...
    int16_t pre_y = ((yh << 8) + yl) - 0x01;
    int8_t data = sys->read_data(pre_y);

    sys->write_gpr(YL, (0x00ff & pre_y));
    sys->write_gpr(YH, (0xff00 & pre_y) >> 8);
    sys->write_gpr(dest, data);
}

void ldd_yq(Sys *sys, int opcode) {
    ...
    int8_t data = sys->read_data((yh << 8) + yl + disp);
    sys->write_gpr(dest, data);
}
...
void xch(Sys *sys, int opcode) {
    ...
    int8_t reg_val = sys->read_gpr(src);
    int8_t data_val = sys->read_data((zh << 8) + zl);

    sys->write_data((zh << 8) + zl, reg_val);
    sys->write_gpr(src, data_val);
}
...
void brlt(Sys *sys, int opcode) {

    if(sys->read_sreg(SF) == 0x00)
        return;
...
void bset(Sys *sys, int opcode) {

    int s_bit = extract(opcode, 4, 7, 0);
    sys->write_sreg(s_bit, 0x01);
}
