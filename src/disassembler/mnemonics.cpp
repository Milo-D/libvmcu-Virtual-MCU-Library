...
string brcc(int opcode) {
    ...
    stream << "brcc " << sign << offs;
    stream << fill(stream.str().size());

    stream << "; (C = 0): PC <- PC " << sign;
    stream << " 0x" << hex << offs << " + 1";

    return stream.str();
}

string brcs(int opcode) {
...
string swap(int opcode) {
    ...
    stream << "; R" << dest << "[7:4] <- R" << dest << "[3:0],";
    stream << " R" << dest << "[3:0] <- R" << dest << "[7:4]";
